...
from Products.Quills import MetaWeblogAPI, BloggerAPI
...
class Migration(object):
    ...
    def migrate(self):
        ...
        print >> self.out, "Migrating %s weblogs..." % len(weblogs)
        for weblog in weblogs:
            self.migrateWeblog(weblog.getObject())

    def migrateWeblog(self, weblog):
        u"""Migrate a weblog."""
        print >> self.out, 'Migrating Weblog: %s...' % weblog.Title()
        ...
        weblog.blogger = BloggerAPI.BloggerAPI().__of__(weblog)
        weblog.blogger.setupRPCAuth(RPCAuth)
    ...
    def migrateWeblogEntry(self, weblog, entry, id):
        ...
        * Don't just check EffectiveDate, see if it is actually published.
        ...
        print >> self.out, u"Migrating entry: '%s'" % entry.Title()
