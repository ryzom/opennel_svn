...
typedef std::vector<uint32>		WordVec;
...
public:
  ...
  void Stripify(const WordVec &in_indices, const int in_cacheSize, NvStripInfoVec &allStrips);
...
protected:
  ...
  static NvFaceInfo *FindGoodResetPoint(NvFaceInfoVec &faceInfos, NvEdgeInfoVec &edgeInfos);

   
  void FindAllStrips(NvStripInfoVec &allStrips, NvFaceInfoVec &allFaceInfos, NvEdgeInfoVec &allEdgeInfos, int numSamples);
  void SplitUpStripsAndOptimize(const NvStripInfoVec &allStrips, NvStripInfoVec &outStrips, NvEdgeInfoVec& edgeInfos);
  
  static bool FindTraversal(NvFaceInfoVec &faceInfos, NvEdgeInfoVec &edgeInfos, NvStripInfo *strip, NvStripStartInfo &startInfo);
  static int  CountRemainingTris(std::list<NvStripInfo*>::iterator iter, std::list<NvStripInfo*>::iterator  end);
  static void CommitStrips(NvStripInfoVec &allStrips, const NvStripInfoVec &strips);
  static float AvgStripSize(const NvStripInfoVec &strips);
  static int FindStartPoint(NvFaceInfoVec &faceInfos, NvEdgeInfoVec &edgeInfos);
  
  void UpdateCache(VertexCache* vcache, NvStripInfo* strip);
  float CalcNumHits(VertexCache* vcache, NvStripInfo* strip);
