...
void CGenericObjectPool<T>::initialize(uint32 initialSize, uint32 minSize,
	...
	m_AllObjects = new CGenericObjectHolder<T>*[initialSize];
...
void CGenericObjectPool<T>::growObjectQueue() {
	...
	newObjList = new CGenericObjectHolder<T>*[newObjSize];

	// Copy over existing pointers. 
	memcpy(newObjList,m_AllObjects,sizeof(CGenericObjectHolder<T>*)*m_CurrentSize);
	...
	if(m_AllocationType == CGenericObjectPool<T>::ALLOC_LAZY) {
		...
		memset(&m_AllObjects[m_CurrentSize], 0, sizeof(CGenericObjectHolder<T>*)*growthNum);
		// TODO henri:everyone is there a better way to add these to the free list?
...
typename CGenericObjectPool<T>::Holder *CGenericObjectPool<T>::createObject(uint32 idx) {
	typename CGenericObjectPool<T>::Holder *oh = new CGenericObjectHolder<T>();
	oh->setObj(new T());
	oh->setIdx(idx);
	return oh;
}
